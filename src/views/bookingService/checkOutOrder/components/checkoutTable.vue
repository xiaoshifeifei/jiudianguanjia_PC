    ...
    <div class="page-container">
      ...
      >
        ...
        <el-table-column
          ...
          :resizable="false"
        >
          <template slot-scope="scope">{{ scope.row.OrderStatus==0?'待接单':scope.row.OrderStatus==1?'已完成':scope.row.OrderStatus==-1?'已取消':scope.row.OrderStatus==2?'待服务':'' }}</template>
        </el-table-column>
        <!-- <el-table-column
          label="操作时间"
          align="center"
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              ...
              type="text"
              @click="handleUpdate(scope.$index,scope.row.OrderStatus,scope.row.OrderSerialNo ,scope.row.OrderID)"
            >接受</el-button>
            <el-button
              v-if="scope.row.OrderStatus===0"
              type="text"
...
import { checkOutList, checkOuttChange, checkOutDetail } from '@/api/room-relevant'
import CheckoutDialog from '@/views/bookingService/checkOutOrder/components/checkoutDialog.vue'
export default {
  name: 'Basetable',
  components: {
    CheckoutDialog
  ...
  methods: {
    async GetRoutineList(val) {
      ...
      let result = await checkOutList(data)
      this.isClean = false
      console.log(result)
    ...
    async handleDetails(row, id) {
      let that = this
      let result = await checkOutDetail({
        OrderID: id
      })
