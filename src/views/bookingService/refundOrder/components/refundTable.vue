    ...
    <div class="page-container">
      ...
      >
        ...
        <el-table-column
          ...
          :resizable="false"
        >
          <template slot-scope="scope">{{ scope.row.OrderStatus==0?'待清扫':scope.row.OrderStatus==1?'已完成':scope.row.OrderStatus==-1?'已取消':'' }}</template>
        </el-table-column>
        <el-table-column
          label="操作时间"
          align="center"
...
import { GetRepairList, ChangeRepair, RepairDetails } from '@/api/room-relevant'
import RefundDialog from '@/views/bookingService/refundOrder/components/refundDialog.vue'
export default {
  name: 'Basetable',
  components: {
    RefundDialog
